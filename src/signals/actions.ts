...
export function deriveQualitativeEvaluation(
  ...
  function dim(
    ...
    const grade = score >= 80 ? "PASS" : score >= 40 ? "PARTIAL" : "FAIL";
    return {
      id,
      label,
      grade,
  ...
  const clarityScore =
    (hasRiskFactors ? 34 : 0) + (hasRecs ? 33 : 0) + (hasFraudTypes ? 33 : 0);
  dims.push(
    dim(
      "output_clarity",
      "Output Clarity",
  ...
  const hasActionKeywords =
    /sar|escalat|file|report|block|freeze|investigat/.test(recText);
  ...
  dims.push(
    dim(
      ...
      cs.sar_ready === true
        ? "Report is marked SAR-ready by the compliance pipeline."
        : cs.sar_ready === false
          ? "Report explicitly marked as NOT SAR-ready."
          : "SAR readiness status was not determined.",
      ["compliance_status.sar_ready"],
      cs.sar_ready !== true
        ? ["Review compliance phase output for SAR eligibility criteria."]
        : [],
    ),
  );
  ...
  dims.push(
    dim(
      ...
      cs.gdpr_compliant === true
        ? "Report is marked GDPR-compliant."
        : cs.gdpr_compliant === false
          ? "Report flagged as non-GDPR-compliant."
          : "GDPR compliance status was not determined.",
      ["compliance_status.gdpr_compliant"],
      cs.gdpr_compliant !== true
        ? ["Verify PII handling and data minimisation in the compliance phase."]
        : [],
    ),
  );
  ...
  dims.push(
    dim(
      ...
      cs.dora_compliant === true
        ? "Report is marked DORA-compliant."
        : cs.dora_compliant === false
          ? "Report flagged as non-DORA-compliant."
          : "DORA compliance status was not determined.",
      ["compliance_status.dora_compliant"],
      cs.dora_compliant !== true
        ? ["Ensure ICT risk and incident reporting fields are populated."]
        : [],
    ),
  );
  ...
  dims.push(
    dim(
      ...
      agentScore < 100
        ? [
            "Review agent findings for conflicting evidence and resolution method.",
          ]
        : [],
    ),
  );
  ...
  const pipeScore =
    failed === 0 && completed >= 13
      ? 100
      : failed === 0 && completed >= 8
        ? 60
        : failed > 0
          ? 20
          : 0;
  dims.push(
    dim(
      ...
      [
        "processing_metadata.phases_completed",
        "processing_metadata.phases_failed",
      ],
      pipeScore < 100
        ? [
  ...
  dims.push(
    dim(
      ...
      evidScore < 100
        ? [
            ...(auditLen < 5
              ? ["Ensure all pipeline phases log to the audit trail."]
              : []),
  ...
  return {
    overallGrade:
      overallScore >= 80 ? "PASS" : overallScore >= 50 ? "PARTIAL" : "FAIL",
    overallScore,
    computedAt: new Date(),
    dimensions: dims,
  };
 ...
 * the same 16-phase pipeline, using dataset-specific performance metrics
...
export function derivePerDatasetEvaluations(
  ...
  for (const dsName of dc.datasets_analyzed) {
    ...
    const recScore =
      m.recall >= 0.8 ? 100 : m.recall >= 0.65 ? 65 : m.recall >= 0.5 ? 30 : 0;
    dims.push(
      dim(
        "recall",
        "Recall",
    ...
    const f1Score =
      f1Val >= 0.82 ? 100 : f1Val >= 0.68 ? 65 : f1Val >= 0.55 ? 30 : 0;
    dims.push(
      dim(
        "f1_score",
        "F1 Score",
    ...
    const aucScore =
      aucVal >= 0.9 ? 100 : aucVal >= 0.75 ? 65 : aucVal >= 0.6 ? 30 : 0;
    dims.push(
      dim(
        "auc_roc",
        "AUC-ROC",
    ...
    const seconds = timeStr
      ? parseFloat(timeStr.replace(/[^0-9.]/g, "")) || 0
      : 0;
    const effScore =
      seconds <= 0
        ? 50 // unknown — neutral
